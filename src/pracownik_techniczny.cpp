// pracownik_techniczny.cpp - Dostosowany do komunikacji po ID
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <sys/shm.h>
...
void obsluga_wstrzymaj(int sig) { (void)sig; wstrzymane = 1; }
void obsluga_wznow(int sig) { (void)sig; wstrzymane = 0; }
void obsluga_ewakuacja(int sig, siginfo_t *info, void *context) { (void)sig; (void)context; ewakuacja = 1; }
...
void proces_stanowiska(int sektor_id, int stanowisko_id, int shm_id, int sem_id, int msg_id) {
    ...
    struct sigaction sa; sa.sa_sigaction = obsluga_ewakuacja; sa.sa_flags = SA_SIGINFO; sigemptyset(&sa.sa_mask); sigaction(SIGRTMIN, &sa, NULL);
    ...
    while (1) {
        ...
        if (liczba_na_stanowisku > 0) {
            ...
            // ZMIANA: Wysyłaj na kanał (ID + 1), nie PID
            ...
            printf("[Stanowisko %d-%d] Wpuszczono %d (Sektor: %d)\n", sektor_id, stanowisko_id, kibic_id, sektor_id);
